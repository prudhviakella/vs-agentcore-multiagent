...
def _load_langsmith_from_ssm() -> None:
    ...
    try:
        ...
        log.info(f"[Knowledge] LangSmith loaded from SSM")
    except Exception as e:
        log.warning(f"[Knowledge] LangSmith not available — tracing disabled: {e}")
...
async def _ensure_agent():
    ...
    async with _agent_lock:
        ...
        log.info(f"[Knowledge] Cold start — building agent")
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            ...
            log.info(f"[Knowledge] OpenAI key loaded")
        ...
        log.info(f"[Knowledge] Agent ready")
...
async def handler(payload: dict, context: BedrockAgentCoreContext):
    ...
    payload:
        message    — the query
        session_id — user thread_id (namespaced by supervisor: {thread_id}__knowledge)
        domain     — "pharma"
    ...
    yield {
        "type": "span",
        "data": {"agent": "knowledge", "elapsed_ms": elapsed},
    }
